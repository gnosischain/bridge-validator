...
async fn test_create_xdai_message_uses_correct_bridge_address() {
    ...
    // Convert config bridge address to hex for comparison
    let expected_bridge = hex::encode(config.eth_xdai_bridge_address.as_slice());

    assert_eq!(
        bridge_in_msg, expected_bridge,
        "Bridge address in message should match config.eth_xdai_bridge_address"
    );
}
...
async fn test_read_process_write_back_to_false_when_not_finalized() {
    ...
    // Mock beacon chain response with finalized block at 50
    // This means blocks above 50 are NOT finalized
    let beacon_response = serde_json::json!({
        "data": {
            "message": {
                "body": {
    ...
    let test_log = create_test_log(
        100,
        "0xf234567890123456789012345678901234567890123456789012345678901234",
    );
    let log_json = serde_json::to_value(&test_log).unwrap();
    ...
    .bind(&log_json)
    .bind(100i64)
    .bind("0xf234567890123456789012345678901234567890123456789012345678901234")
    .bind("false")
    .bind(0i32)
    .execute(&pool)
    .await
    ...
    // Step 1: Read from database (this marks is_processed = true)
    let result = processor.read_from_db().await.unwrap();
    assert!(result.is_some());
    let event_log = result.unwrap();
    let entry_id = event_log.id;
    let block_number = event_log.block_number.unwrap();

    // Verify it's marked as processed in the database
    let db_row_before: (String,) = sqlx::query_as(
        r#"
        SELECT is_processed
        FROM event_logs
        WHERE id = $1
    ...
    .unwrap();
    assert_eq!(
        db_row_before.0, "true",
        "Entry should be marked as processed after read_from_db"
    );
    ...
    // Step 3: Write is_processed back to false (simulating the behavior in process_message_or_skip)
    processor
        .write_is_processed_to_false(entry_id)
        .await
        .unwrap();

    // Step 4: Verify is_processed is set back to false in the database
    let db_row_after: (String,) = sqlx::query_as(
        r#"
        SELECT is_processed
        FROM event_logs
        WHERE id = $1
    ...
    .unwrap();
    assert_eq!(
        db_row_after.0, "false",
        "Entry should be marked as NOT processed after write_is_processed_to_false"
    );

    // Step 5: Verify the entry can be read again (since it's now false)
    let result2 = processor.read_from_db().await.unwrap();
    assert!(result2.is_some(), "Entry should be available to read again");
    let event_log2 = result2.unwrap();
    assert_eq!(event_log2.id, entry_id, "Should read the same entry again");
}

// Additional test to verify the full flow with a finalized block
#[tokio::test]
async fn test_read_stays_processed_when_finalized() {
    ...
    // Mock beacon chain response with finalized block at 200
    // This means blocks below or equal to 200 are finalized
    let beacon_response = serde_json::json!({
        "data": {
            "message": {
                "body": {
    ...
    let test_log = create_test_log(
        100,
        "0xf334567890123456789012345678901234567890123456789012345678901234",
    );
    let log_json = serde_json::to_value(&test_log).unwrap();
    ...
    .bind(&log_json)
    .bind(100i64)
    .bind("0xf334567890123456789012345678901234567890123456789012345678901234")
    .bind("false")
    .bind(0i32)
    .execute(&pool)
    .await
    ...
    .await
    .unwrap();
    assert_eq!(db_row_before.0, "true");
    ...
    // Step 3: Do NOT call write_is_processed_to_false since block is finalized
    // This simulates the normal flow where finalized blocks stay processed

    // Step 4: Verify is_processed is still true
    let db_row_after: (String,) = sqlx::query_as(
        r#"
        SELECT is_processed
        FROM event_logs
    ...
    assert_eq!(
        db_row_after.0, "true",
        "Entry should remain processed for finalized blocks"
    );

    // Step 5: Verify the entry cannot be read again (since it's still true)
    let result2 = processor.read_from_db().await.unwrap();
    assert!(
        result2.is_none(),
        "Entry should NOT be available to read again"
    );
}
